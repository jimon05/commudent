...
현재 Commudent는 별도 계정 승인 없이 `/`에서 바로 접근할 수 있는 공개 데모 모드로 동작합니다.

- 첫 방문자는 홈 대시보드에서 샘플 발표 리포트를 바로 볼 수 있습니다.
- `/onboarding`은 로그인 없이 로컬 프로필을 만드는 선택 흐름입니다.
- Supabase 환경변수가 없거나 로그인하지 않은 사용자의 데이터는 브라우저 localStorage에 저장됩니다.
- Supabase를 연결하면 로그인 사용자는 클라우드 저장을 사용할 수 있지만, 공개 접근 자체를 막지는 않습니다.
...
Vercel production은 환경변수 없이도 공개 데모로 배포할 수 있습니다. 실제 STT, 표현 추천, 사용자별 클라우드 저장까지 운영하려면 아래 값을 등록합니다.
...
공개 데모 배포만 필요하면 이 단계는 건너뛸 수 있습니다. 실제 분석/저장 기능을 운영하려면 Vercel Project Settings -> Environment Variables에 아래 값을 production 기준으로 등록합니다.
...
- `/`: 공개 홈 대시보드 접근
- `/onboarding`: 계정 없이 Profile Setup
...
- [ ] 실제 STT/표현 추천/클라우드 저장을 운영한다면 Vercel에 `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`를 등록했다.
- [ ] 클라우드 저장을 운영한다면 Supabase SQL을 최신 상태로 실행했다.
- [ ] 클라우드 저장을 운영한다면 `recordings` Storage bucket과 RLS policy가 적용됐다.
- [ ] 로그인 기능을 운영한다면 Supabase Auth redirect URL에 Vercel production URL을 등록했다.
- [ ] 로그인 기능을 운영한다면 테스트 계정을 생성하고 Profile Setup에서 이메일 로그인/가입을 확인했다.
